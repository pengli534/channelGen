...
3. 按提示：
   ...
   - 生成自定义样例时，可输入 `IN_num / OUT_num / T_num / Nsample`，默认值为 `4 / 4 / 1 / 2`
    - 选择要处理的 `.mat`
   - 当 `IN_num = OUT_num` 且为偶数维 MIMO 时，可选择是否开启 `MIMO双向测试`
    - 选择归一化、自动/手动 IQ_max
...
- `validation_custom_bidi.mat`: 自定义结构样例，运行时输入尺寸参数；非对角线子信道全0，对角线子信道仅第1个tap为 `delay=0, real=1, imag=0`
...
- `MIMO双向测试` 仅在 `IN_num = OUT_num` 时显示，并且仅对偶数维方阵 MIMO 生效
- 若开启 `MIMO双向测试`，则按输入维、输出维各自均分为 `2 x 2` 四个子矩阵，所有非对角块子信道的全部 taps 都置为 0
- 开启 `MIMO双向测试` 后，输出 MAT 中的浮点 `H`、定点 `Hq`、`.irc`、`.ird` 都基于清零后的结果
...
- `xxx_fixedpoint.mat` 的 `meta` 中会记录 `mimo_bidi_enabled` 和 `mimo_bidi_supported`
...
- 使用 `verify_ird_file`（保持原文件名）进行交互式校验：
  ...
  - 当检测到 `meta.mimo_bidi_enabled = 1` 时，还会额外检查 `H` 和 `Hq` 的非对角块是否已全部清零
